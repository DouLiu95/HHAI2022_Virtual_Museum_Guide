class QuestionPaser:
    ...
    def sql_transfer(self, question_type, entities):
        ...
        if question_type == 'paintings_collection':
            sql = ["MATCH (p:Paintings)-[r:in_COLLECTION]->(c:Collection) where p.name ='{0}' RETURN c ".format(i)
                   for i in entities]

        elif question_type == 'paintings_creator':
            sql = ["MATCH (p:Paintings)-[r:has_CREATOR]->(c:Person) where p.name ='{0}' RETURN c ".format(i)
                   for i in entities]

        elif question_type == 'paintings_exhibition':
            sql = ["MATCH (p:Paintings)-[r:in_EXHIBITION]->(c:Exhibition) where p.name ='{0}' RETURN c".format(i)
                   for i in entities]

        elif question_type == 'paintings_material':
            sql = ["MATCH (p:Paintings)-[r:on_MATERIAL]->(c:Material) where p.name ='{0}' RETURN c ".format(i)
                   for i in entities]

        elif question_type == 'paintings_genre':
            sql = ["MATCH (p:Paintings)-[r:belongsto_GENRE]->(c:Genre) where p.name ='{0}' RETURN c".format(i)
                   for i in entities]

        elif question_type == 'paintings_depicts':
            sql = ["MATCH (p:Paintings)-[r:has_KEYWORD]->(c:Keyword) where p.name ='{0}' RETURN c ".format(i)
                   for i in entities]

        elif question_type == 'paintings_movement':
            sql = ["MATCH (p:Paintings)-[r:belongsto_MOVEMENT]->(c:Movement) where p.name ='{0}' RETURN c ".format(i)
                   for i in entities]
            ...
            sql = ["MATCH (p:Paintings) where p.name ='{0}' RETURN p.description ".format(i)
                   for i in entities]

        elif question_type == 'paintings_country':
            sql = ["MATCH (p:Paintings)-[r:in_COUNTRY]->(c:Country) where p.name ='{0}' RETURN c ".format(i)
                   for i in entities]


        elif question_type == 'collection_country':
            sql = ["MATCH (p:Collection)-[r:in_COUNTRY]->(c:Country) where p.name ='{0}' RETURN c ".format(i)
                   for i in entities]
        ...
        #            for i in entities]
            ...
            sql = ["MATCH (p:Paintings)-[r:in_COLLECTION]->(c:Collection) where c.name ='{0}' RETURN p order by p.sitelink limit 3".format(i)
                   for i in entities]
            ...
            sql = ["MATCH (p:Paintings)-[r:belongsto_GENRE]->(c:Genre) where c.name ='{0}' RETURN p order by p.sitelink limit 3 ".format(i)
                   for i in entities]

        elif question_type == 'depicts_paintings':
            sql = ["MATCH (p:Paintings)-[r:has_KEYWORD]->(c:Keyword) where c.name ='{0}' RETURN p order by p.sitelink limit 3 ".format(i)
                   for i in entities]
