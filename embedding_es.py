...
def get_similarity(name,exception_list,label =None):
        ...
        for i in exception_list:
            if i != []:
                if isinstance(i,list):
                    for item in i:
                        if item != name: # in case we exclude the entity the user is searching
                            embedding = embedding[(embedding.uri != item)]
                elif isinstance(i,str):
                    if i != name: # in case we exclude the entity the user is searching
                        embedding = embedding[(embedding.uri != i)]
    ...
    if label:
        ...
        try:
            curent_id = int(embedding[(embedding.uri == name)].nodeId)
            test = list(embedding[(embedding.uri == name)].embedding)
        except:
            curent_id = int(embedding[(embedding.uri == name)].iloc[0].nodeId)
            test = list(embedding[(embedding.uri == name)].iloc[0].embedding)
        print(curent_id)
        similarity_score = []
        ...
        try:
            ...
            curent_id = int(embedding[(embedding.uri == name)].iloc[0].nodeId)
            test = list(embedding[(embedding.uri == name)].iloc[0].embedding)
        label = embedding[(embedding.uri == name)].label.item()
