...
class ArtGraph:
    def __init__(self):
        ...
        self.history = pd.DataFrame(columns = [ 'session',
                                    'intent',
                                    'queryText',
                                    'parameters',
                                                'parameters_dic','item_preference',
    ...
    def show_exhibit(self,exhibit_in_dialog):
        ...
        #     return data
            ...
            sparql_for_name = "MATCH (n:Person) WHERE n.uri = '" + str(exhibit_in_dialog) + "' RETURN n"
            data = self.g.run(str(sparql_for_name)).data()
            print(data)
            return data
        else:
    ...
    def transfer_request_to_dict(self,request_json, item_preference = '', returned_entity= '',Cypher = None ):
        ...
        d = {}
        parameters_dic = request_json['queryResult']['parameters']
        ...
        d["session"] = request_json["session"]
        d['intent'] = request_json['queryResult']['intent']['displayName']
        d['queryText'] = request_json['queryResult']['queryText']
        d['item_preference'] = item_preference
        d['response.entity'] = returned_entity
        d['parameters'] = parameter_list
        d['parameters_key'] = key_para
        ...
        if d['session'] in self.history['session'].to_list():
            d['exhibit_checked'] = self.history[self.history['session'].isin([d['session']])].iloc[-1]['exhibit_checked']
            d['exhibit_unchecked'] = self.history[self.history['session'].isin([d['session']])].iloc[-1]['exhibit_unchecked']
            d['preference'] = self.history[self.history['session'].isin([d['session']])].iloc[-1]['preference']
        else:
            d['exhibit_checked'] = []
    ...
    def decide_label_preference(self,mode,session):
        ...
        '''
        if session in self.history['session'].to_list():
            preference = self.history[self.history['session'].isin([session])].iloc[-1]['preference']
        else:
    ...
    def get_neighbors(self,name,type='',hop=1):
        ...
        if name.startswith('Q'):
            name = r"http://www.wikidata.org/entity/" + name
        if type != '':
            type = ':'+type
        cypher_neighbors = "MATCH (p"+type+ "{uri: \""+name+"\"}) CALL apoc.neighbors.athop(p, \"belongsto_GENRE|has_CREATOR|belongsto_MOVEMENT|has_KEYWORD|on_MATERIAL|in_COLLECTION|in_EXHIBITION\", +"+str(hop)+") YIELD node RETURN node"
